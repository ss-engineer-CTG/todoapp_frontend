...
export const useTaskDrag = (timelineRef?: React.RefObject<HTMLDivElement>) => {
  ...
  const handleDragMove = useCallback((e: MouseEvent) => {
    ...
    requestAnimationFrame(() => {
      ...
      // タイムラインの自動スクロール
      let timelineContent: HTMLElement | null = null;
      
      if (timelineRef && timelineRef.current) {
        timelineContent = timelineRef.current;
      } else {
  ...
  }, [dragInfo, zoomLevel, dispatch, timelineRef]);
  ...
  const handleDragEnd = useCallback(() => {
    ...
    dispatch(endDrag());
  }, [dragInfo, dispatch, showFeedback, handleDragMove]);
  ...
  return {
    ...
    dragRef,
    isDragging: !!dragInfo && (
      (!!dragInfo.projectId && !!dragInfo.taskId) ||
      (!!dragInfo.projectId && !!dragInfo.taskId && !!dragInfo.subtaskId)
    ),
    dragInfo
  };
};