    ...
    <div className={`p-4 rounded-lg border ${
      ...
      {notifications.length > 0 && (
        <div className={`mb-6 p-3 rounded-lg border ${
          resolvedTheme === 'dark' ? 'bg-yellow-900/20 border-yellow-800' : 'bg-yellow-50 border-yellow-200'
        }`}>
          ...
          <div className="space-y-1">
            {notifications.slice(0, 3).map((notification, index) => (
              ...
              >
                <span className={resolvedTheme === 'dark' ? 'text-yellow-100' : 'text-yellow-700'}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        {/* 完了率 */}
        <div className={`p-3 rounded-lg border ${
          resolvedTheme === 'dark' ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'
        }`}>
          <div className="flex items-center space-x-2 mb-1">
            <CheckCircle size={16} className="text-green-600" />
            <span className={`text-xs font-medium ${
              resolvedTheme === 'dark' ? 'text-green-200' : 'text-green-700'
            }`}>
              完了率
            </span>
          </div>
        ...
        <div className={`p-3 rounded-lg border ${
          ...
          <div className={`text-xl font-bold ${
            resolvedTheme === 'dark' ? 'text-yellow-100' : 'text-yellow-900'
          }`}>
        ...
        {/* 平均進捗 */}
        <div className={`p-3 rounded-lg border ${
          resolvedTheme === 'dark' ? 'bg-purple-900/20 border-purple-800' : 'bg-purple-50 border-purple-200'
        }`}>
          <div className="flex items-center space-x-2 mb-1">
            <TrendingUp size={16} className="text-purple-600" />
            <span className={`text-xs font-medium ${
              resolvedTheme === 'dark' ? 'text-purple-200' : 'text-purple-700'
            }`}>
              平均進捗
            </span>
          </div>
          ...
          <div className="space-y-2">
            {lifecycleState.expiredGoals.slice(0, 3).map((goal) => (
              <div key={goal.id} className="flex items-center justify-between text-sm">
                <span className={resolvedTheme === 'dark' ? 'text-red-100' : 'text-red-700'}>
                  {goal.title}
                </span>
      ...
      {lifecycleState.lastArchiveDate && (
        <div className={`p-3 rounded-lg border ${
          resolvedTheme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'
        }`}>
